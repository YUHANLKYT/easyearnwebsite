...
function verifyApiToken(payload: ParsedPostback, headerApiToken?: string | null): boolean {
  const configuredToken = process.env.REVTOO_APP_TOKEN?.trim();
  const configuredAppId = process.env.REVTOO_APP_ID?.trim();
...
function verifyHash(payload: ParsedPostback, rawUrl: string, rawBody: string): boolean {
  const secrets = [
    process.env.REVTOO_POSTBACK_SECRET?.trim(),
    process.env.REVTOO_APP_SECRET?.trim(),
    process.env.REVTOO_SERVER_TO_SERVER_KEY?.trim(),
    process.env.REVTOO_SECRET_KEY?.trim(),
  ].filter((value): value is string => Boolean(value));
...
async function handleCredit(payload: ParsedPostback, payoutCents: number) {
  ...
  const offerTitle = payload.offerTitle ?? "Revtoo Offer";
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (pendingDays > 0) {
        await tx.transaction.create({
          data: {
            ...
            description: `${buildPendingEarnNotice(payoutCents, pendingDays)} (Revtoo tx: ${payload.tx})`,
          },
        });
        return;
      }
      ...
      await tx.transaction.create({
        data: {
          ...
          type: "EARN",
          amountCents: payoutCents,
          description: `Revtoo reward credited (tx: ${payload.tx})`,
        },
      });
    });
  } catch (error) {
...
async function handleReversal(payload: ParsedPostback) {
  ...
  const taskKey = `revtoo:${payload.tx}`;
  const reversalDescription = `Revtoo reward reversed (tx: ${payload.tx})`;
  const now = new Date();
      ...
      if (!claim.creditedAt) {
        ...
        await tx.transaction.create({
          data: {
            ...
            type: "EARN_PENDING",
            amountCents: 0,
            description: `Revtoo pending reward canceled (tx: ${payload.tx})`,
          },
        });
        return;
      }
