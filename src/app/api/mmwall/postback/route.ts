...
function getHashCandidates(rawUrl: string): string[] {
  const stripped = stripQueryKeys(rawUrl, new Set(["hash", "signature", "sig", "secure_hash", "token"]));
...
function verifyApiToken(payload: ParsedPostback, headerApiToken?: string | null): boolean {
  const configuredToken = process.env.MMWALL_APP_TOKEN?.trim();
  const configuredAppId = process.env.MMWALL_APP_ID?.trim();
  const configuredApiKey = process.env.MMWALL_API_KEY?.trim();
  if (!configuredToken && !configuredAppId && !configuredApiKey) {
    return true;
  }
  ...
  if (providedValues.length < 1) {
    // Some offerwalls do not include a token in callback payloads.
    return true;
  }

  const candidates = [configuredToken, configuredAppId, configuredApiKey]
    .filter((value): value is string => Boolean(value))
...
function verifyHash(payload: ParsedPostback, rawUrl: string, rawBody: string): boolean {
  const secrets = [
    process.env.MMWALL_POSTBACK_SECRET?.trim(),
    process.env.MMWALL_APP_SECRET?.trim(),
    process.env.MMWALL_SERVER_TO_SERVER_KEY?.trim(),
    process.env.MMWALL_SECRET_KEY?.trim(),
  ].filter((value): value is string => Boolean(value));
  ...
  const amountCandidates = [payload.amountRaw, payload.grossRaw]
    .filter((value): value is string => Boolean(value))
    .map((value) => value.trim())
    .filter((value) => value.length > 0);

  if (subId && transId && amountCandidates.length > 0) {
    for (const secret of secrets) {
      const formulaCandidates = amountCandidates
        .flatMap((amount) => [
          createHash("md5").update(`${subId}${transId}${amount}${secret}`).digest("hex"),
          createHash("md5").update(`${decodeIfPossible(subId)}${transId}${amount}${secret}`).digest("hex"),
          createHash("md5").update(`${subId}:${transId}:${amount}:${secret}`).digest("hex"),
          createHash("md5").update(`${subId}:${amount}:${secret}`).digest("hex"),
          createHash("md5").update(`${transId}:${amount}:${secret}`).digest("hex"),
          createHmac("sha256", secret).update(`${subId}${transId}${amount}`).digest("hex"),
        ])
        .map(normalizeHash);
...
function parsePostback(searchParams: URLSearchParams): ParsedPostback {
  return {
    ...
    apiToken: getParam(searchParams, ["api_key", "apikey", "app_token", "app_id", "appid", "token"]),
  };
}
...
async function handleCredit(payload: ParsedPostback, payoutCents: number) {
  ...
  const now = new Date();
  const taskKey = `mmwall:${payload.tx}`;
  ...
  const offerTitle = payload.offerTitle ?? "MMWall Offer";
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (pendingDays > 0) {
        await tx.transaction.create({
          data: {
            ...
            description: `${buildPendingEarnNotice(payoutCents, pendingDays)} (MMWall tx: ${payload.tx})`,
          },
        });
        return;
      }
      ...
      await tx.transaction.create({
        data: {
          ...
          type: "EARN",
          amountCents: payoutCents,
          description: `MMWall reward credited (tx: ${payload.tx})`,
        },
      });
    });
  } catch (error) {
...
async function handleReversal(payload: ParsedPostback) {
  ...
  const taskKey = `mmwall:${payload.tx}`;
  const reversalDescription = `MMWall reward reversed (tx: ${payload.tx})`;
  const now = new Date();
      ...
      if (!claim.creditedAt) {
        ...
        await tx.transaction.create({
          data: {
            ...
            type: "EARN_PENDING",
            amountCents: 0,
            description: `MMWall pending reward canceled (tx: ${payload.tx})`,
          },
        });
        return;
      }
