...
import { getGiftbitCatalogEntryById } from "@/lib/giftbit";
...
export async function POST(request: Request) {
  ...
  const entry = getGiftbitCatalogEntryById(catalogId);
  if (!entry) {
  ...
  const customName = `Giftbit: ${entry.product}`;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.transaction.create({
        data: {
          ...
          description: `Giftbit request: ${entry.product} (${requestedLocalAmount}, charged ${formatUSD(amountCents)} pending admin review)`,
        },
      });
    });
  } catch (error) {
  ...
  redirect(
    buildRedirect(redirectTo, {
      notice: `Giftbit request submitted for ${entry.product} (${requestedLocalAmount}, charged ${formatUSD(amountCents)}).`,
    }),
  );
}
